...
def obb2poly_np(rbboxes, version='oc', type=''):
    """Convert oriented bounding boxes to polygons.
    ...
    if version == 'oc':
        ...
        results = obb2poly_np_le90(rbboxes, type=type)
    else:
        raise NotImplementedError
    return results
...
def obb2poly_np_le90(obboxes, type=''):
    """Convert oriented bounding boxes to polygons.
    ...
    polys = get_best_begin_point(polys, type=type)
    return polys
...
def get_best_begin_point(coordinates, type=''):
    ...
    """
    if type == 'zjw':
        coordinates = get_best_begin_point_single(coordinates)
