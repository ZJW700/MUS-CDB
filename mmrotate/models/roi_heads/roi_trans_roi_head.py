...
class RoITransRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def _bbox_forward_train(self, stage, x, sampling_results, gt_bboxes,
        ...
        bbox_results = self._bbox_forward(stage, x, rois)         # predicted value for each roi:cls_score([1024,16]), bbox_pred([1024,5]), bbox_feats([1024,256,7,7])
        ...
        if img_metas[0]['label_type'] == 'partial':
            ...
            with torch.no_grad():
                bg_scores = self.simple_test(x, selected_bboxes, img_metas, return_bg_score=True)   # list(tensor*2)(512,)
    ...
    def simple_test(self, x, proposal_list, img_metas, return_split=False, return_entropy=False, return_bg_score=False, return_aug_bbox_result=False, rescale=False):
        ...
        if return_entropy:
            return det_bboxes[0], det_labels[0], det_entropys, bg_scores
